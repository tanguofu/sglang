#!/usr/bin/env python3
"""Multi-dimension eval suite — bypass router, hit worker directly via kubectl port-forward.

Since router circuit breaker is flaky (50% no_available_workers), we test
worker-direct to get true model quality. We use kubectl exec to run curl
inside the worker pod, capturing the streaming response.
"""
...
EVAL_CASES = [
    ...
    {"id": "R3", "category": "reasoning",
     ...
     "expect_regex": r"\b9\b", "max_tokens": 100},
    ...
    {"id": "L1", "category": "long_context",
     ...
     "expect_regex": r"Papaya", "max_tokens": 50},
    # ========== 10. Creative ==========
    ...
    {"id": "S1", "category": "summarization",
     ...
     "expect_regex": r"Apollo\s*11|moon|Armstrong", "max_tokens": 200},
]

def call_worker(prompt, max_tokens=300, tools=None, tool_choice=None):
    """Call worker-direct via kubectl exec curl."""
    body = {
        "model": MODEL,
    ...
    # Write body to a temp file inside the pod to avoid shell escaping issues
    cmd = [
        "kubectl", "exec", "-n", "kube-system", WORKER_POD, "--",
        "/usr/bin/curl", "-sS", "-N", "--max-time", "120",
        "http://127.0.0.1:30000/v1/messages",
    ...
    for line in out.splitlines():
        ...
        if t == "content_block_delta":
            ...
            if delta.get("type") == "text_delta":
                ...
                tool_chunks.append(delta.get("partial_json", ""))
        elif t == "message_delta":
            stop_reason = evt.get("delta", {}).get("stop_reason")
    return "".join(text_chunks), "".join(tool_chunks), stop_reason

def check(case, text, tool_text):
    expect = case.get("expect_regex")
    if not expect:
        return True, "no expect"
    full = text + "\n" + tool_text
    if re.search(expect, full, re.IGNORECASE | re.DOTALL | re.MULTILINE):
        return True, "matched"
    return False, f"no match for /{expect}/"

print("=" * 80)
print(f"GLM-5.2 Eval (worker-direct, bypass router) — {len(EVAL_CASES)} cases")
print(f"Worker: {WORKER_POD} (127.0.0.1:30000)")
print("=" * 80)
...
for case in EVAL_CASES:
    ...
    try:
        text, tool_text, stop = call_worker(
            case["prompt"],
        ...
        ok, note = check(case, text, tool_text)
        results.append({"id": cid, "category": cat, "ok": ok, "note": note,
                        "elapsed": round(elapsed, 2), "stop": stop,
                        "text": text[:500], "tool_text": tool_text[:200]})
        ...
        elapsed = time.perf_counter() - start
        results.append({"id": cid, "category": cat, "ok": False,
                        "note": f"error: {e}", "elapsed": round(elapsed, 2),
                        "text": "", "tool_text": ""})
        by_cat.setdefault(cat, []).append(False)
...
with open("/tmp/eval_results_worker.json", "w") as f:
    json.dump(results, f, indent=2, ensure_ascii=False)
print(f"\nFull results saved to /tmp/eval_results_worker.json")
