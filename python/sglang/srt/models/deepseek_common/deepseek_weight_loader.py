...
def _load_fused_indexer_wk(
    ...
    Returns False when projection fusion is disabled and there is no fused
    param, so the caller falls through to per-tensor loading. The same loader
    serves CUDA full fusion and ROCm projection-only fusion.
    """
    ...
    def do_load_weights(
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for name, loaded_weight in weights:
                ...
                # Projection fusion packs wk + weights_proj into one BF16 param; the
