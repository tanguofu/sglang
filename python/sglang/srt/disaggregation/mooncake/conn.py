...
class MooncakeKVManager(CommonKVManager):
    ...
    def _transfer_data(self, mooncake_session_id, transfer_blocks):
        ...
        _has_map = hasattr(self, "_gpu_to_host_map")
        _staging = os.environ.get("SGLANG_PD_HOST_STAGING") == "1"
        if _staging or _has_map:
            logger.info(
                f"_transfer_data: staging={_staging} has_map={_has_map} "
                f"map_size={len(self._gpu_to_host_map) if _has_map else 0} "
                f"blocks={len(transfer_blocks)} first_src={hex(int(transfer_blocks[0][0]))}"
            )

        if _staging and _has_map:
            import ctypes
            ...
            for src_addr, dst_addr, length in transfer_blocks:
                ...
                if host_src is not None:
                    # KV cache hit: copy GPU data into the pre-registered
                    # host staging buffer at the matching offset, then
                    # transfer from there.  No per-transfer register/deregister.
                    ret = hip_lib.hipMemcpy(
                    ...
                    # STAGING MISS: address not in pre-registered map (e.g.
                    # state/aux data). Allocate per-transfer host buffer,
                    # hipMemcpy D2H, register, transfer, then deregister+free.
                    host_ptr = ctypes.c_void_p()
                    alloc_ret = hip_lib.hipMallocHost(
                        ctypes.byref(host_ptr), ctypes.c_size_t(length)
                    )
                    if alloc_ret != 0:
                        ...
                        if cp_ret != 0:
                            ...
                            miss_host_ptrs.append(host_ptr.value)
                            miss_host_cptrs.append(host_ptr)
                            miss_lens.append(length)
                            final_src_addrs.append(host_ptr.value)
            if miss_host_ptrs:
                self.engine.batch_register(miss_host_ptrs, miss_lens)
            ret = self.engine.batch_transfer_sync(
                mooncake_session_id, final_src_addrs, list(dst_addrs), list(lengths)
            )
            if miss_host_ptrs:
                self.engine.batch_deregister(miss_host_ptrs)
                for cp in miss_host_cptrs:
    ...
    def send_kvcache_slice(
        ...
        def process_layer_tp_aware(src_layer_ptr, dst_layer_ptr):
            ...
            transfer_blocks = list(zip(src_addr_list, dst_addr_list, length_list))
            return self._transfer_data(mooncake_session_id, transfer_blocks)
