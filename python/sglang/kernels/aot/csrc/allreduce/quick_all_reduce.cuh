...
// guard is applied. The opaque intrinsic makes the scaled f32 values explicit
// inputs and prevents that transform.
// gfx942 port note: upstream #34484 used inline asm (v_cvt_pk_f16_f32), which
// is a CDNA4 (gfx950+) instruction and fails to assemble on gfx942. The
// __floats2half2_rn intrinsic is equally opaque to the reassociation and
// compiles on both CDNA3 and CDNA4.
__quickreduce_device_inline__ half2 scaled_bfloat162_to_half2(nv_bfloat162 value, float scale) {
  ...
  scaled.x *= scale;
  scaled.y *= scale;
  return __floats2half2_rn(scaled.x, scaled.y);
}
