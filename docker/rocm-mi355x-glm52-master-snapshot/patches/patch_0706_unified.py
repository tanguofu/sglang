...
# ============================================================
def patch_06d_disable_dsa_fused_store():
    print("\n" + "="*60)
    print("Running 06d: Disable DSA fused-store on ROCm")
    print("="*60)

    path = "/sgl-workspace/sglang/python/sglang/jit_kernel/fused_store_index_cache.py"
    with open(path, "r") as f:
        content = f.read()

    if "[FIX] DSA fused store disabled" in content:
        print("[SKIP] DSA fused-store already disabled")
        return

    start = content.find("def can_use_dsa_fused_store(")
    if start < 0:
        errors.append("06d can_use_dsa_fused_store function not found")
        print("[ERROR] can_use_dsa_fused_store function not found")
        return

    end = content.find("\n\n@debug_kernel_api", start)
    if end < 0:
        errors.append("06d fused-store function end marker not found")
        print("[ERROR] fused-store function end marker not found")
        return

    replacement = '''def can_use_dsa_fused_store(
    key_dtype: torch.dtype, indices_dtype: torch.dtype, page_size: int
) -> bool:
    # [FIX] DSA fused store disabled on ROCm/MI355X.
    # The ROCm fused-store kernel compiles after the fp8 pack patch, but A/B
    # testing showed long-context GLM-5.2 output corruption and MTP accept-rate
    # collapse once it writes index K cache. Keep the safe fallback path:
    # act_quant(key) + set_index_k_scale_buffer(...).
    return False
'''
    ...
    with open(path, "w") as f:
        f.write(content)
    print("[OK] DSA fused-store disabled: can_use_dsa_fused_store -> False")
...
def main():
    ...
    run_bundle_01_05()
    patch_06d_disable_dsa_fused_store()
