...
def _build_kernel_paged_mfma_r_w(*, num_heads: int, head_size: int,
                                 block_kv: int, rows_per_block: int,
                                 waves_per_block: int,
                                 page_size: int = 1,
                                 preshuffle: bool = False,
                                 convert_q_fn: bool = False,
                                 convert_kv_fn: bool = False):
    """Paged FlyDSL kernel for the DSA decode/target-verify MQA logits path.

    The kernel supports page_size=1 and AITER preshuffle page_size=16/32/64.
    RPB consecutive query rows must belong to the same request (for example,
    NEXTN's four draft rows). Those rows share one page table and one KV window,
    so an RPB=4 kernel can reuse each KV tile across four MFMA rows just like
    ...
    assert BKV % 16 == 0, f"block_kv={BKV} must be a multiple of 16 for MFMA"
    assert D % 32 == 0, f"head_size={D} must be a multiple of 32 for MFMA K32"
    assert RPB in (1, 2, 4), "paged FlyDSL supports RPB in {1, 2, 4}"
    if preshuffle:
        assert page_size % 16 == 0, (
            f"preshuffle page_size={page_size} must be a multiple of 16"
        )
    else:
        assert page_size == 1, "non-preshuffle paged FlyDSL requires page_size=1"
    N_TILES = BKV // 16
    assert N_TILES % WPB == 0, (
        f"BKV/16={N_TILES} must be divisible by waves_per_block={WPB}"
    )
    ...
    if convert_kv_fn:
        _cvt_tag += "_ck"
    _kname = (
        f"fp8_paged_mqa_logits_H{H}_D{D}_bkv{BKV}_mfma_r{RPB}_w{WPB}"
        f"_p{page_size}{'_ps' if preshuffle else ''}{_cvt_tag}_flydsl"
    )
    ...
    def kernel(
        ...
        seq_len_kv: fx.Int32,
        stride_logits_s: fx.Int32,
        stride_block_tables: fx.Int32,
        stride_kv_scales: fx.Int32,
        num_splits: fx.Int32,
    ):
        f32_0 = arith.constant(0.0, type=T.f32)
        mfma_res_ty = Vec.make_type(4, fx.Float32)
        ...
        sc_t = GTensor(
            ...
            dtype=T.f32,
            shape=(-1, -1),
            stride=(stride_kv_scales, fx.Int32(1)),
        )
        w_t = GTensor(weights, dtype=T.f32, shape=(-1, H))
        ctx_t = GTensor(context_lens, dtype=T.i32, shape=(-1,))
        bt_t = GTensor(
            block_tables,
            dtype=T.i32,
            shape=(-1, -1),
            stride=(stride_block_tables, fx.Int32(1)),
        )
        out_t = GTensor(
            logits,
            dtype=T.f32,
        ...
        for j in range_constexpr(RPB):
            ...
            for mi in range_constexpr(M_TILES):
                h_a = _i32_add(fx.Int32(mi * 16), lane_mod_16)
                row_h = _i32_add(
                    fx.Int32(arith.muli(_to_raw(row), _to_raw(fx.Int32(H)))),
                    h_a,
                )
                base_a = fx.Int32(arith.muli(_to_raw(row_h), _to_raw(fx.Int32(D))))
                for kk in range_constexpr(K_STEPS):
        ...
        with ir.InsertionPoint(tile_loop.body):
            ...
            for ni in range_constexpr(N_TILES_PER_WAVE):
                ...
                page_col = fx.Int32(
                    arith.divui(_to_raw(col_clamped), _to_raw(fx.Int32(page_size)))
                )
                token_col = fx.Int32(
                    arith.remui(_to_raw(col_clamped), _to_raw(fx.Int32(page_size)))
                )
                page_idx = fx.Int32(bt_t[r0, page_col])
                kv_scales_tile[ni] = _to_raw(fx.Float32(sc_t[page_idx, token_col]))
                base_b = fx.Int32(
                    arith.muli(_to_raw(page_idx), _to_raw(fx.Int32(D + 4)))
                )
                if preshuffle:
                    base_b = fx.Int32(
                        arith.muli(
                            _to_raw(page_idx),
                            _to_raw(fx.Int32(page_size * (D + 4))),
                        )
                    )
                    token_group = fx.Int32(
                        arith.divui(_to_raw(token_col), _to_raw(fx.Int32(16)))
                    )
                    token_in_group = fx.Int32(
                        arith.remui(_to_raw(token_col), _to_raw(fx.Int32(16)))
                    )
                    group_stride = fx.Int32((D // 16) * 256)
                    base_b = _i32_add(
                        base_b,
                        _i32_add(
                            fx.Int32(
                                arith.muli(_to_raw(token_group), _to_raw(group_stride))
                            ),
                            fx.Int32(
                                arith.muli(
                                    _to_raw(token_in_group),
                                    _to_raw(fx.Int32(16)),
                                )
                            ),
                        ),
                ...
                for kk in range_constexpr(K_STEPS):
                    ...
                    if preshuffle:
                        h_chunk = fx.Int32(
                            arith.divui(_to_raw(d_b), _to_raw(fx.Int32(16)))
                        )
                        h_in = fx.Int32(
                            arith.remui(_to_raw(d_b), _to_raw(fx.Int32(16)))
                        )
                        physical_d = _i32_add(
                            fx.Int32(
                                arith.muli(_to_raw(h_chunk), _to_raw(fx.Int32(256)))
                            ),
                            h_in,
                        )
                    raw = _load_pack_i64(kv_i32, _i32_add(base_b, physical_d))
                    b_packs[ni][kk] = _fn_to_fnuz_i64(raw) if convert_kv_fn else raw
    ...
    @flyc.jit
    def launch_fp8_paged_mqa_logits_mfma_r_w(
        Q: fx.Tensor,
        KV: fx.Tensor,
        kv_scales: fx.Tensor,
        weights: fx.Tensor,
    ...
    return launch_fp8_paged_mqa_logits_mfma_r_w
...
def _mk_paged_builder(rpb, wpb):
    return lambda **kw: _build_kernel_paged_mfma_r_w(
        **kw, rows_per_block=rpb, waves_per_block=wpb)


_PAGED_VARIANT_BUILDERS = {
    f"mfma_r{r}_w{w}": _mk_paged_builder(r, w)
    for r in (1, 2, 4)
    for w in (1, 2, 4)
}
PAGED_KERNEL_VARIANTS = tuple(_PAGED_VARIANT_BUILDERS.keys())
PAGED_DEFAULT_VARIANT = "mfma_r4_w4"
...
def compile_fp8_paged_mqa_logits(
    *,
    ...
    head_size: int,
    block_kv: int = _BLOCK_KV,
    variant: str = PAGED_DEFAULT_VARIANT,
    page_size: int = 1,
    preshuffle: bool = False,
    convert_q_fn: bool = False,
    convert_kv_fn: bool = False,
):
    """Return a cached, compiled paged FlyDSL launcher."""
    if variant not in _PAGED_VARIANT_BUILDERS:
        raise ValueError(
            f"unknown paged fp8_mqa_logits variant {variant!r}; "
            f"available: {list(PAGED_KERNEL_VARIANTS)}"
        )
    launcher = _PAGED_VARIANT_BUILDERS[variant](
        num_heads=num_heads,
        head_size=head_size,
        block_kv=block_kv,
        page_size=page_size,
...
def flydsl_fp8_paged_mqa_logits(
    ...
    max_seq_len,
    stream=None,
    variant=PAGED_DEFAULT_VARIANT,
    page_size=1,
    preshuffle=False,
):
    """FlyDSL gfx942 paged FP8 MQA logits for DSA caches.

    Q and context_lens use one entry per query row. For NEXTN, pass the four
    draft rows of each request consecutively and repeat each request's page
    table row four times; this lets ``mfma_r4_w4`` reuse one KV tile for four
    rows. The output is a [rows, max_seq_len] view with a 256-element-aligned
    row stride for the fused top-k kernel.
    """
    if Q.dim() != 3:
        raise ValueError(f"Q must be [rows, heads, head_dim], got {tuple(Q.shape)}")
    if not Q.is_contiguous():
        raise ValueError("Q must be contiguous")
    rows, num_heads, head_size = Q.shape
    if page_size == 1:
        if kv_scales.dim() == 1:
            kv_scales = kv_scales.view(-1, 1)
        elif kv_scales.dim() != 2 or kv_scales.shape != (KV.shape[0] // (head_size + 4), 1):
            raise ValueError("kv_scales must be [tokens, 1] for page_size=1")
    elif kv_scales.dim() != 2 or kv_scales.shape[1] != page_size:
        raise ValueError(
            f"kv_scales must be [blocks, {page_size}], got {tuple(kv_scales.shape)}"
        )
    if kv_scales.stride(1) != 1:
        raise ValueError("kv_scales must have unit stride along page_size")
    num_blocks = kv_scales.shape[0]
    num_tokens = num_blocks * page_size
    if KV.dim() == 2:
        if KV.shape != (num_tokens, head_size + 4):
            raise ValueError(
                f"KV must be [tokens, head_dim+4], got {tuple(KV.shape)}"
            )
        KV = KV.reshape(-1)
    elif KV.dim() != 1 or KV.shape[0] != num_tokens * (head_size + 4):
        raise ValueError(
            "KV must be [tokens, head_dim+4] or its flat view and match kv_scales"
        )
    if not KV.is_contiguous():
        raise ValueError("KV must be contiguous")
    if weights.shape != (rows, num_heads):
        raise ValueError(f"weights must be [{rows}, {num_heads}], got {tuple(weights.shape)}")
    if not weights.is_contiguous():
        raise ValueError("weights must be contiguous")
    if context_lens.shape != (rows,):
        raise ValueError(f"context_lens must be [{rows}], got {tuple(context_lens.shape)}")
    if not context_lens.is_contiguous():
        raise ValueError("context_lens must be contiguous")
    if block_tables.dim() != 2 or block_tables.shape[0] != rows:
        raise ValueError(
            f"block_tables must be [{rows}, blocks], got {tuple(block_tables.shape)}"
        )
    required_block_cols = (max_seq_len + page_size - 1) // page_size
    if block_tables.shape[1] < required_block_cols:
        raise ValueError(
            f"block_tables width {block_tables.shape[1]} is shorter than "
            f"required {required_block_cols} blocks"
        )
    if block_tables.stride(1) != 1:
        raise ValueError("block_tables must have unit stride along max_seq_len")
    ...
    if preshuffle:
        ...
        raise ValueError("non-preshuffle paged FlyDSL requires page_size=1")

    _fnuz = torch.float8_e4m3fnuz
    _fn = torch.float8_e4m3fn
    if Q.dtype not in (_fnuz, _fn) or KV.dtype not in (_fnuz, _fn):
        raise ValueError(f"Q/KV must be e4m3 fp8, got {Q.dtype}, {KV.dtype}")
    ...
    rpb = int(variant.split("_")[1][1:])
    if rows % rpb != 0:
        raise ValueError(
            f"paged variant {variant} requires rows ({rows}) to be divisible by {rpb}"
        )

    weights = weights.reshape(rows, num_heads)
    context_lens = context_lens.reshape(rows)
    aligned_size = 256
    seq_len_kv_aligned = (
        (max_seq_len + aligned_size - 1) // aligned_size * aligned_size
    )
    logits = torch.empty(
        (rows, seq_len_kv_aligned), dtype=torch.float32, device=Q.device
    )

    device_index = Q.device.index
    if device_index is None:
        device_index = torch.cuda.current_device()
    num_splits = _auto_num_splits(rows, max_seq_len, rpb, device_index)
    if stream is None:
    ...
    with torch.cuda.device(device_index):
        _run_compiled(
            ...
            int(kv_scales.stride(0)),
            int(num_splits),
            stream,
        )
    return logits[:, :max_seq_len]
