...
Used by patch_gdr_flush.py. Safe to import from scheduler workers.
"""
...
HSACO = os.environ.get(
    "SGLANG_PD_GDR_FLUSH_HSACO",
    "/data/mooncake-patched/gdr_l2_flush.hsaco",
)
...
def _launch(fn, gpu_id: int) -> bool:
    ...
    rc = hip.hipModuleLaunchKernel(
        fn, 1, 1, 1, 32, 1, 1, 0, None, None, None
    )
    if rc != 0:
...
def ensure_read_sink(engine, gpu_id: int = 0) -> Optional[int]:
    """64B host sink for the 1-byte/8-byte loopback RDMA READ."""
    ...
    if rc != 0 or not ptr.value:
        ...
        return None
    # The object handed in here is the sglang MooncakeTransferEngine wrapper,
    # which exposes batch_register/register (NOT register_memory — that lives
    # on the inner C++ engine). Try the wrapper API first, then the raw engine,
    # so the read-sink actually registers and the RDMA READ flush fires.
    reg = None
    registered = False
