...
Worker 镜像 `YOUR_REGISTRY/your-project/sglang-glm52-308x:v0517-gdr-kernel-v1` 已把 GDR peermem、L2 flush、DSA/HIP 补丁烤进镜像，启动脚本不再依赖 `/data/mooncake-patched` overlay。`/data` hostPath 只挂模型权重。
...
| `statefulset/sglang-1p1d-decode` | node-NODE_DECODE_0_IP | TP8 decode，NEXTN spec（steps=2, draft=3；accept 已顶满） |
