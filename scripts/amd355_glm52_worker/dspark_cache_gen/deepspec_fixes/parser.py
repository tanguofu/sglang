...
class GeneralParser:
    ...
    def parse(
        ...
        # Use string find instead of regex (handles special token chars with |)
        search_start = 0
        while True:
            content_start_char = conversation_text.find(self.assistant_message_separator, search_start)
            if content_start_char == -1:
                break
            content_start_char += len(self.assistant_message_separator)
            ...
            eot = self.chat_template.end_of_turn_token or ""
            if eot:
                content_end_char = conversation_text.find(eot, content_start_char)
                if content_end_char == -1:
                    content_end_char = len(conversation_text)
            else:
                content_end_char = len(conversation_text)
