...
LOG_FILE="/data/v10_4node_train.log"
CONTAINER_NAME="glm52_dspark_v10_4node"
GID_INDEX=1

echo "[$(date)] Launching DSpark v10 4-node 256-expert training (rank=$NODE_RANK)"

ALL_PEER_IPS="144.202.61.0 216.128.158.18 216.128.154.57 149.28.114.238"
...
docker run -d --name ${CONTAINER_NAME} \
  --device /dev/kfd --device /dev/dri --device /dev/infiniband --network host --shm-size 64G --ipc host \
  ...
  -e NCCL_DEBUG=WARN -e NCCL_TIMEOUT=7200 \
