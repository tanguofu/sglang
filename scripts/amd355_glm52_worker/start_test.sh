#!/bin/bash
# Usage: bash /data/start_test.sh <test_name> <extra_sglang_args...>
# Example: bash /data/start_test.sh cds2 --num-continuous-decode-steps 2
set -e
TEST_NAME="${1:-test}"
shift
IMAGE="lmsysorg/sglang-rocm:v0.5.13.post1-rocm720-mi35x-20260623"
CONTAINER_NAME="sglang_glm52_${TEST_NAME}"
...
docker run -d \
  ...
  bash -c "
    ...
    exec python3 -m sglang.launch_server \
      ...
      --prefill-max-requests 32 --max-prefill-tokens 32768 --kv-cache-dtype fp8_e4m3 \
      --speculative-algorithm NEXTN --speculative-num-steps 2 --speculative-num-draft-tokens 3 \
      --speculative-eagle-topk 1 --max-running-requests 128 --watchdog-timeout 3600 --log-level info $*
  "
echo "Container ${CONTAINER_NAME} started with extra args: $@"
