...
impl Normalizable for ResponsesRequest {
    ...
    fn normalize(&mut self) {
        ...
        // FIX: coerce tool_choice to None when no tools are present.
        // Some clients (e.g. codex CLI) send tool_choice="auto" without a tools
        // array on turns that don't need tool calls. The validator rejects this
        // with a 400 ("tool_choice is only allowed when tools are specified").
        // Per OpenAI spec, tool_choice without tools is valid — the model just
        // generates a normal message. Coerce to None to avoid the spurious 400.
        let has_tools = self.tools.as_ref().is_some_and(|t| !t.is_empty());
        if !has_tools {
            self.tool_choice = Some(ToolChoice::Value(ToolChoiceValue::None));
        }
