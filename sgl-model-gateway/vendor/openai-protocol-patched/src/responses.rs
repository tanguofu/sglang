...
/// Lenient deserializer for `ResponsesRequest.input`.
///
/// Codex and other clients sometimes send input formats that the typed
/// `ResponseInput` enum cannot parse (e.g., novel content block types).
/// This deserializer tries the typed parse first and falls back to
/// `ResponseInput::Text(json_string)` so the request is not rejected at
/// the gateway — the Python worker can still handle it.
